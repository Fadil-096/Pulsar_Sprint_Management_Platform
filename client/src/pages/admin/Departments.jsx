...
export default function Departments() {
      ...
      <div className="flex-1 p-8 text-text-primary h-full overflow-y-auto bg-bg-primary">
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[...Array(4)].map((_, i) => <div key={i} className="h-32 bg-white/5 rounded-xl"></div>)}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 p-8 text-text-primary h-full overflow-y-auto bg-bg-primary">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight mb-2">Departments</h1>
        <p className="text-text-secondary">Department overview and team distribution.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {cards.map((card, idx) => (
          <div key={idx} className="bg-bg-card border-[1px] border-line rounded-2xl p-6 shadow-sm flex items-center justify-between">
            <div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="bg-bg-card border-[1px] border-line rounded-2xl p-6">
          ...
          <div className="divide-y divide-line">
            {stats?.sprintStatusDist?.length > 0 ? stats.sprintStatusDist.map((stat, i) => (
              <div key={i} className="flex items-center justify-between py-3">
                ...
                <span className="text-[14px] font-medium bg-bg-secondary px-2 py-1 rounded-xl">{stat.value}</span>
              </div>
